...
!function (a, b, c, d) {
  ...
  var e = function e(b) {
    this.core = b, this.core.options = a.extend({}, e.Defaults, this.core.options), this.swapping = !0, this.previous = d, this.next = d, this.handlers = { 'change.owl.carousel': a.proxy(function (a) {
        a.property ? 'position' == a.property.name && (this.previous = this.core.current(), this.next = a.property.value) : false;
...
$(document).ready(function () {
	...
	function stick() {
		...
		if (window.matchMedia('(max-width: 960px)').matches) {
			...
			var coordinate = $('nav').offset().top;

			$(window).scroll(function () {

				var element = $('nav');

				if ($(window).scrollTop() >= coordinate) {
					element.addClass('fixed');
					$('.wrapper.top').addClass('fixed');
				} else {
					element.removeClass('fixed');
					$('.wrapper.top').removeClass('fixed');
				}
			});
		}
	};
	...
	$('.tabs a').click(function (e) {
		...
		var tab = $(this).data('tab-handler');
		$(this).addClass('active').siblings().removeClass('active');
		$('[data-tab=\'' + tab + '\']').addClass('active').siblings().removeClass('active');
	});

	$('.subs_block .details').click(function (e) {
		e.preventDefault();
		var text = $(this).text() == 'Подробнее' ? 'Скрыть' : 'Подробнее';
		$(this).text(text);

		$(this).parents('.order').find('.drop_details').toggleClass('active');
	});
});
